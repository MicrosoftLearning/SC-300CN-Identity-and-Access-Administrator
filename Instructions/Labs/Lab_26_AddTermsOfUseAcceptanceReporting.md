...
组织可以通过 Azure AD 使用条款策略这种简单的方法向最终用户显示信息。可以通过这样的呈现方式确保用户看到法律要求或合规性要求的相关免责声明。本文介绍如何开始利用使用条款 (ToU) 策略。

你需要为组织创建并强制执行 ToU 策略。
...
1. 对于本练习，可选择你可能拥有的任何 PDF，或使用 Microsoft Word 创建简单的使用条款文档，然后打印为 PDF。

1. 选择使用条款文档的语言。

1. 可以通过语言选项上传多个版本的使用条款，每个版本的语言各不相同。最终用户看到的使用条款版本取决于其浏览器首选项。
...
1. 在 **“条件访问”** 下，选择 **“自定义策略”**。
    ...
    可以使用自定义条件访问策略将使用条款细化，向下细化到特定云应用程序或用户组。有关详细信息，请参阅 [https://docs.microsoft.com/zh-cn/azure/active-directory/conditional-access/require-tou](https://docs.microsoft.com/en-us/azure/active-directory/conditional-access/require-tou)。
...
1. 在“选择”窗格中，选择要用于测试使用条款策略的帐户。

1. 如果选择管理员帐户，与所有条件访问策略一样，请确保你有另一个具有足够权限的帐户来更改条件访问策略。这是为了确保如果条件访问策略导致不良结果，你的管理员帐户不会被锁定。
